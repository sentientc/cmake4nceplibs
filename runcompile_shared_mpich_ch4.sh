...
module load rcec/ufs-ext/bbc872c-intel19-mpich-ch4
module load rcec/libfabric/1.11.2-intel19
...
#'--with-rdma=gen2' '--enable-rdma-cm' '--enable-threads=multiple' '--enable-romio' '--with-ch3-rank-bits=32' '--with-ib-include=/usr/include' '--with-ib-libpath=/usr/lib64' '--with-ibverbs-include=/usr/include' '--with-ibverbs-lib=/usr/lib64'
export MPITYPE=MPICH3
export wd=`pwd`
export td=/opt/ohpc/pkg/rcec/pkg/NCEPLIBS-external
export tv=bbc872c-intel19-mpich-ch4
export LDFLAGS=-fPIC
mkdir -p build
mkdir -p ${td}/${tv}
rm -rf build/*
cp -f config_netcdfc.sh_nopnetcdf.tmpl config_netcdfc.sh.tmpl
cp -f CMakeLists_nopnetcdf_ch4.txt CMakeLists.txt
cd build
if [ 1 -eq 1 ];then
